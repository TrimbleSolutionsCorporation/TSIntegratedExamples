namespace SpreadsheetReinforcement.Tools
{
    ...
    public static class TxSettings
    {
        ...
        {
            return string.IsNullOrEmpty(componentNumber) ? string.Empty : string.Format("p_{0}", componentNumber);
        }
        ...
        public static FileInfo GetFileByName(string fullName)
        {
            if (!fullName.Contains("."))
                throw new ApplicationException(
                    "GetFileByName argument fullName did not contain '.', must contain extension");
            var shortName = fullName.StringLeft(".");
            var extension = fullName.StringRight(".");
            if (string.IsNullOrEmpty(shortName) || string.IsNullOrEmpty(extension))
                throw new ApplicationException("GetFileByName failed to parse argument fullName into parts.");

            //Check that attribute file exists, set to something viable if possible
            return TxModel.StandardPropertyDirectories
                .SelectMany(di => di.GetFiles(fullName, SearchOption.AllDirectories)).FirstOrDefault();
        }

        /// <summary>
        /// Gets the string to the right of separator
        /// </summary>
        /// <param name="tString">String to parse</param>
        /// <param name="separator">String separator</param>
        /// <returns>Remainder of string to right of separator</returns>
        public static string StringRight(this string tString, string separator)
        {
            var result = string.Empty;
            if (string.IsNullOrEmpty(tString) || string.IsNullOrEmpty(separator)) return result;
            var index = tString.LastIndexOf(separator, StringComparison.Ordinal);
            return index < 1 ? result : tString.Substring(index + 1, tString.Length - index - 1);
        }

        /// <summary>
        /// Gets the string to the left of separator
        /// </summary>
        /// <param name="tString">String to parse</param>
        /// <param name="separator">String separator</param>
        /// <returns>Remainder of string to left of separator</returns>
        public static string StringLeft(this string tString, string separator)
        {
            var result = string.Empty;
            if (string.IsNullOrEmpty(tString) || string.IsNullOrEmpty(separator)) return result;
            var index = tString.LastIndexOf(separator, StringComparison.Ordinal);
