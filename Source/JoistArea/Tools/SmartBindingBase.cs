namespace JoistArea.Tools
{
    ...
    [Guid("A465649F-279D-4FEB-8579-D8C1FFB879C4")]
    public abstract class SmartBindingBase : INotifyPropertyChanged, IDataErrorInfo
    {
        ...
        [XmlIgnore]
        [Browsable(false)]
        public SmartPropertiesValidator Validator = new SmartPropertiesValidator();
            ...
            [CallerMemberName] string propertyName = null)
        {
            ...
            if(string.IsNullOrEmpty(value) && !allowNull) Validator.DataErrors.Add(propertyName, $"{propertyName} must not be blank");
            ...
            [CallerMemberName] string propertyName = null)
        {
            if(string.IsNullOrEmpty(propertyName)) return;
            ClearPropertyErrors(propertyName);
            if(value < minValue) Validator.DataErrors.Add(propertyName, $"{propertyName} must not be greater than {minValue}");
            else if(value > maxValue) Validator.DataErrors.Add(propertyName, $"{propertyName} must be less than {maxValue} ");
        }
    ...
    [Serializable]
    [StructLayout(LayoutKind.Sequential)]
    [Guid("87A59C6C-8FA5-4664-AB11-1B0D133B17E6")]
